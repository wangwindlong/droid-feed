...
class HomeFragment : BaseInjectFragment<EmptyViewModel, FragmentHomeBinding>(EmptyViewModel::class.java, "feed") {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        mViewModel.jump2Fragment.observe(viewLifecycleOwner, EventObserver{ index ->
            ...
            newFragment?.let {
                ft.replace(R.id.simple_fragment, it)
            }
            ft.setTransition(FragmentTransaction.TRANSIT_FRAGMENT_OPEN)
            ft.addToBackStack("HomeFragment\$fragment$index")
            ft.commit()
        })
        return view
    }
